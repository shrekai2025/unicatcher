...
1. **提取抽象风格特征，而非具体内容**
   你需要提取的是**写作方法和风格模式**，而非具体的业务信息或专有名词。

   ✅ 应该提取的（抽象风格特征）：
   - 语气特点：如"直接、简洁"、"幽默调侃"、"理性分析"、"感性叙事"
   - 句式结构：如"先提问后解答"、"使用排比强调"、"善用类比说明"
   - 修辞手法：如"常用反问增强语气"、"喜欢用具体例子支撑观点"
   - 逻辑组织：如"开门见山式"、"层层递进式"、"对比论证式"
   - 表达习惯：如"常用数据支撑"、"善用emoji调节气氛"、"喜欢总结升华"

   ❌ 不应该提取的（具体内容特征）：
   - 专有名词：如"$BTC"、"以太坊"、"OpenAI"、"特朗普"、"AGI"等
   - 项目名称：如"ChatGPT"、"Midjourney"、具体公司名、产品名
   - 人名地名：如"马斯克"、"硅谷"、"北京"等具体人物和地点
   - 具体事件：如"某某发布会"、"某项目融资"等时事热点
   - 数字数据：如"涨了30%"、"市值100亿"等具体数值

   示例对比：
   - 错误："用户常提到$BTC和以太坊，关注加密货币价格波动" ❌
   - 正确："用户善于用具体案例支撑观点，常引用市场数据增强说服力" ✅

2. **关注可迁移的写作模式**
   提取的风格特征应该能够应用到任何话题的写作中，而不是绑定特定主题。

   示例：
   - 错误："开头常说'今天比特币又涨了'" ❌
   - 正确："开头习惯用热点事件引入，快速抓住注意力" ✅

3. **保持风格档案的通用性和可复用性**
   风格档案应该像一个"写作风格模板"，适用于不同话题和内容类型。

4. **具体推文例子仅作为风格参考**
   在examples字段中可以引用原推文片段来说明风格特征，但不要将这些例子中的专有名词提取到风格描述中。

5. **每个分析点都要有具体的推文例子支撑**
   确保所有风格特征都有实际推文支撑，但要抽象化描述。
