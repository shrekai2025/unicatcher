...
请以JSON格式返回结果：
{
  ...
  async generateText(prompt: string): Promise<string> {
    try {
      console.log(`[智谱AI文本生成] 开始生成，模型: ${this.model}`);
      ...
      console.log(`[智谱AI文本生成] 生成完成，内容长度: ${content.length}`);
      return content;

    } catch (error) {
      console.error('[智谱AI文本生成] 生成失败:', error);
      throw new Error(`智谱AI文本生成失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
